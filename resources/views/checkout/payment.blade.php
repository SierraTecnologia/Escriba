...
@section('store-content')
    ...
    <div class="row">
        ...
        <div class="col-md-4">
            ...
            @if (escritor()->customer()->hasProfile() && ! is_null(escritor()->customer()->lastCard('card_last_four')))
                <form method="post" action="{{ route('siravel.process.last-card') }}">
                    {!! csrf_field() !!}
                    <button class="btn btn-outline-primary" id="lastCardBtn" type="submit">Pay with card (ending in {!! escritor()->customer()->lastCard('card_last_four') !!})</button>
                </form>
            @endif
        </div>
    </div>
