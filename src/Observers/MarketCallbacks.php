...
class EscritorCallbacks
{
    ...
    public function handle($event, $payload): bool
    {
        ...
        // Ignore
        if (!app()->bound(EscritorService::class)) {
            return true;
        }
        ...
        if ($method == 'onBooting') {
            $model::addGlobalScope(new EscritorScope);
            return true;
        }

        if ($method == 'onCreating') {
            $business = app()->make(EscritorService::class);
